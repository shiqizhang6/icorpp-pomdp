...
class PomdpGenerator(object):
    ...
    def __init__(self, num_item, num_person, num_room, r_max, r_min, strategy, \
        ...
        # writing to files
        self.filename = 'models/' + time.strftime('%Y%m%d') + '_' + strategy + '_old.pomdp'
        self.reward_mat = reward_mat_bin
        self.writeToFile()

        self.filename = 'models/' + time.strftime('%Y%m%d') + '_' + strategy +'_new.pomdp'
...
def main():
    ...
    # old values that the robot is not so cautious
    # r_max = 20.0
    # r_min = -20.0

    # new values that the robot prefers to ask more questions
    r_max = 30.0
    r_min = -30.0
