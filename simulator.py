...
class Simulator(object):
    ...
    def run_numbers_of_trials(self):
        ...
        f_i = open('reward_results_i.txt', 'w')
        f_p = open('reward_results_p.txt', 'w')
        f_r = open('reward_results_r.txt', 'w')
        f_i.write(string_i)
        f_p.write(string_p)
        f_r.write(string_r)
        f_i.close()
...
def main():
    ...
    s = Simulator(uniform_init_belief = False, 
        ...
        auto_observations = True, 
        print_flag = False, 
        policy_file = 'policy/new_cautious.policy', 
        pomdp_file = 'models/20160718_413_new.pomdp',
        trials_num = 100000,
        num_item = 4, 
        num_person = 1, 
        num_room = 3)
